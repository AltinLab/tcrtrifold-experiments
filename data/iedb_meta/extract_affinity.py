...
SELECT as_type_id,
    ...
    units,
    obi_id,
    distinct_receptor.distinct_receptor_id as receptor_id,
    epitope.linear_peptide_seq,
    reference.reference_id
FROM tcell
    JOIN reference on tcell.reference_id = reference.reference_id
    JOIN assay_type ON tcell.as_type_id = assay_type.assay_type_id
...
-- WHERE assay_type.assay_type = "surface plasmon resonance (SPR)";
"""
...
II = (
    ...
    .unnest("chains")
).select(pl.exclude("mhc_struct", "mhc_name_inferred", "split_parts"))

iedb_metadat = pl.concat([I, II])

iedb_metadat.write_parquet("triad_affinity.parquet")
