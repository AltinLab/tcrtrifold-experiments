#!/usr/bin/env python
import tcrdock
import tcrdock.pdblite
import tcrdock.tcrdock_info
...
def extract_tcrdock_geom(
    ...
    for i, sample in enumerate(samples):
        ...
        tdinfo = tcrdock.tcrdock_info.TCRdockInfo().from_sequences(
            ...
            tcr_aseq,
            tcr_bseq,
            anarci_cdrs=True,  # use anarci for CDRs
        )
...
if __name__ == "__main__":
    ...
    # run this once to trigger db generation
    # so the rest can run in parallel
    tdinfo = tcrdock.tcrdock_info.TCRdockInfo().from_sequences(
        df.select("mhc_1_species")[0].item(),
        1 if df.select("mhc_class")[0].item() == "I" else 2,
        df.select("mhc_1_seq")[0].item(),
        df.select("mhc_2_seq")[0].item(),
        df.select("peptide")[0].item(),
        df.select("tcr_1_seq")[0].item(),
        df.select("tcr_2_seq")[0].item(),
        anarci_cdrs=True,
    )
