...
process FASTA_TO_YAML_WITH_TEMPLATES {
    label "boltz_local"
    ...
    output:
    tuple val(meta), path("*.yaml"), path("*.cif"), optional: true

    script:
    def segids = (meta.containsKey('segids')) ? "--segids ${meta.segids.join(',')}" : ''
    def skip_msa_arg = meta.containsKey("skip_msa") ? "--skip_msa ${meta.skip_msa.join(',')}" : ''
    """
    compose_inference_YAML_with_templates.py \\
        -jn "${meta.id}" \\
        --fasta_path ${fasta} \\
        --inf_dir "$inf_dir" \\
        ${skip_msa_arg} \\
...
process BOLTZ_INFERENCE_WITH_TEMPLATES {
    label "boltz_cpu"

    input:
    tuple val(meta), path(yaml), path(cif)
...
workflow BOLTZ_FROM_TRIAD_PARQUET_WITH_TEMPLATES {
    ...
    triad_fasta_channel = SEQ_LIST_TO_FASTA(triad_channel)
    triad_yaml_channel = FASTA_TO_YAML_WITH_TEMPLATES(triad_fasta_channel, triad_inf_dir)
    
    traid_meta_inf = BOLTZ_INFERENCE_WITH_TEMPLATES(triad_yaml_channel)
