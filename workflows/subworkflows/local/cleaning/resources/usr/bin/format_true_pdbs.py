...
def tcrtrifold_fmt_pdb(row, output_path):
    ...
    for segid_colname, curated_fasta_seq_colname, rename_segid in zip(
        ...
        rg = u.select_atoms(f"segid {segid} and name CA and record_type ATOM").residues

        if curated_fasta_seq_colname != "peptide" and row["peptide"] in rg.sequence(
            format="string"
        ):
            rg = u.select_atoms(
                f"segid {segid} and name CA and record_type ATOM"
            ).residues
            rg_seq = rg.sequence(format="string")
            rg_idx = rg_seq.index(row["peptide"])
            rg = rg[rg_idx + len(row["peptide"]) :]
