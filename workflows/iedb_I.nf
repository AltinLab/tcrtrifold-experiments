/*
Parameters:
- input: Path to the IEDB I raw data CSV file
*/
...
include { CLEAN_IEDB_I } from './subworkflows/local/cleaning'
...
include { TCRDOCK_GEOM_FROM_INFERENCE } from './subworkflows/local/extract_feat'
...
workflow {
    ...
    validation_exclusions = Channel.fromPath(params.validation_exclusions)
    ...
    CLEAN_IEDB_I(raw_data_csv)
    triad_cleaned = CLEAN_IEDB_I.out.triad_parquet
    pmhc_cleaned = CLEAN_IEDB_I.out.pmhc_parquet

    EXCLUDE_VALIDATION_TRIADS(triad_cleaned, pmhc_cleaned, validation_exclusions)

    triad_cleaned = EXCLUDE_VALIDATION_TRIADS.out.annot_triad_parquet
    triad_excluded = EXCLUDE_VALIDATION_TRIADS.out.excluded_triad_parquet
    pmhc_cleaned = EXCLUDE_VALIDATION_TRIADS.out.remaining_pmhc_parquet
